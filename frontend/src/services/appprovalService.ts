import type {  CreateApprovalLogRequest, UpdateApprovalLogRequest  } from "../interfaces/appproval";
...
// สร้างผลงานใหม่
export async function CreateApprovalLog(data: CreateApprovalLogRequest) {
    console.log("Creating approvalLog with payload:", data);
    return await axios
        .post(`${apiUrl}/approvalLog`, data, getRequestOptions())
        .then((res) => res)
        .catch((e) => {
            console.error("Create approvalLog error:", e.response?.data);
            return e.response;
        });
}
...
export async function GetApprovalLog() {
    return await axios
        .get(`${apiUrl}/approvalLog/my`, getRequestOptions()) // สมมติว่ามี route นี้
        .then((res) => res)
        .catch((e) => e.response);
}


export async function UpdateApprovalLog(id: number, data: UpdateApprovalLogRequest) {
    return await axios
        .patch(`${apiUrl}/approvalLog/${id}`, data, getRequestOptions())
        .then((res) => res)
        .catch((e) => e.response);
}

export async function DeleteApprovalLog(id: number) {
    return await axios
        .delete(`${apiUrl}/approvalLog/${id}`, getRequestOptions())
        .then((res) => res)
        .catch((e) => e.response);
}
